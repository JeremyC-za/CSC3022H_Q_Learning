...
CNeuralNet::CNeuralNet(uint inputLayerSize, uint hiddenLayerSize, uint outputLayerSize, double lRate, double mse_cutoff) :
	...
	inputLayerSize(inputLayerSize), hiddenLayerSize(hiddenLayerSize), outputLayerSize(outputLayerSize), lRate(lRate), mse_cutoff(mse_cutoff)

{
	inputLayer.resize(inputLayerSize);
	hiddenLayer.resize(hiddenLayerSize);
	hiddenError.resize(hiddenLayerSize);
	outputLayer.resize(outputLayerSize);
	outputError.resize(outputLayerSize);

	hiddenLayerWeights.resize(hiddenLayerSize);
	outputLayerWeights.resize(outputLayerSize);
...
void CNeuralNet::initWeights(){
	for (int i = 0; i < hiddenLayerSize; i++){
		for (int j = 0; j < inputLayerSize; j++){
			float randWeight = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 1.0f)) - 0.5f;
			hiddenLayerWeights[i][j] = randWeight;
		}
	}
	for (int i = 0; i < outputLayerSize; i++){
...
void CNeuralNet::feedForward(std::vector<double> inputs) {
	...
	for (int i = 0; i < hiddenLayerSize; i++){
		...
		temp = 0;
		for (int j = 0; j < inputLayerSize; j++){
			temp = inputLayer[j] * hiddenLayerWeights[i][j];
			sum += temp;
		}
		hiddenLayer[i] = (1 / (1 + pow(e, (-1*sum))));
	}
	...
	for (int i = 0; i < outputLayerSize; i++){
		...
		temp = 0.0f;
		for (int j = 0; j < hiddenLayerSize; j++){
			temp = hiddenLayer[j] * outputLayerWeights[i][j];
			sum += temp;
		}
		outputLayer[i] = (1 / (1 + pow(e, (-1*sum))));
	}
}
/**
...
void CNeuralNet::propagateErrorBackward(std::vector<double> desiredOutput){
	...
	//output layer error
	for (int i = 0; i < outputLayerSize; i++){
		error = outputLayer[i] * (1 - outputLayer[i]) * (desiredOutput[i] - outputLayer[i]);
		outputError[i] = error;
	}
	//hidden layer error
	for (int i = 0; i < hiddenLayerSize; i++){
		error = hiddenLayer[i] * (1 - hiddenLayer[i]);
		double sum = 0;
		for (int j = 0; j < outputLayerSize; j++){
			sum += outputLayerWeights[j][i] * outputError[j];
		}
		error = error * sum;
		hiddenError[i] = error;
	}
	...
	for (int i = 0; i < outputLayerSize; i++){
		for (int j = 0; j < hiddenLayerSize; j++){
			double change = lRate * outputError[i] * hiddenLayer[j];
			outputLayerWeights[i][j] += change;
		}
	}
	//hidden layer weight
	for (int i = 0; i < hiddenLayerSize; i++){
		for (int j = 0; j < inputLayerSize; j++){
			double change = lRate * hiddenError[i] * inputLayer[j];
			hiddenLayerWeights[i][j] += change;
		}
	}
}
/**
...
double CNeuralNet::meanSquaredError(std::vector<double> desiredOutput){
	...
	double err = 0;
	for (int i = 0; i < outputLayerSize; i++){
		double error = desiredOutput[i] - outputLayer[i];
		sum += error*error;
	}
	err = sum / outputLayerSize;
	return err;
}
/**
...
void CNeuralNet::train(std::vector<std::vector<double> > inputs, std::vector<std::vector<double> > outputs, uint trainingSetSize) {
	...
	while (true){
		...
		MSE = MSE / trainingSetSize;
		std::cout << "Error = " << MSE << " after " << iterations << " iterations." << std::endl;
		if (MSE <= mse_cutoff) break;
	}
}
/**
